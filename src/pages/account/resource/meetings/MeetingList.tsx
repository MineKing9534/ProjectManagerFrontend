...
export default function MeetingList({ parent }: { parent?: Resource }) {
	...
	const { state: createState, error: createError, post } = useRest(`/${ parent?.resourceType.toLowerCase() }s/${ parent?.id }/meetings`, {
		onSuccess: () => {
			onClose()
			get()
		},
	...
	const { state, data, error, get } = useRest<PaginationResult<Meeting>>(`${ parent ? `/${ parent?.resourceType.toLowerCase() }s/${ parent?.id }` : !user.admin ? "/users/@me" : "" }/meetings?page=${ page }&entriesPerPage=5`, {
		auto: true,
		onError: onErrorOpen
	})
